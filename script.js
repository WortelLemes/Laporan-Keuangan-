// Global Variables
let transactions = []; // Data akan dimuat dari Firestore secara real-time
...
// Fungsi untuk mengkonversi string mata uang ke angka (misal: 'Rp1.234' -> 1234)
function parseCurrency(currencyString) {
    if (!currencyString) return 0;
    // Hapus semua karakter non-angka kecuali koma/titik (lalu hilangkan lagi)
    // Ini mengasumsikan format 'RpX.XXX.XX'
    const cleanedString = currencyString.replace(/[^0-9]/g, '');
    return parseInt(cleanedString) || 0;
}

// Fungsi untuk animasi angka (counting up/down)
function animateValue(obj, start, end, duration) {
    ...
    const step = (timestamp) => {
        ...
        // Hitung nilai saat ini berdasarkan progres
        const value = start + (end - start) * progress;
        
        // Memformat angka saat animasi. Gunakan Math.round() untuk mencegah angka desimal panjang
        obj.textContent = formatCurrency(Math.round(value));
        
        if (progress < 1) {
            window.requestAnimationFrame(step);
        } else {
            // Pastikan nilai akhir yang diformat ditampilkan dengan benar
            obj.textContent = formatCurrency(end);
            
            // Perbarui warna berdasarkan nilai akhir (khusus untuk total saldo)
            if (obj.id === 'current-balance') {
                if (end < 0) {
                    obj.classList.remove('text-gray-100');
                    obj.classList.add('text-red-500');
                } else {
                    obj.classList.remove('text-red-500');
                    obj.classList.add('text-gray-100');
                }
            }
        }
    };
    
    // Perbarui warna secara instan jika nilainya berubah drastis (hanya untuk saldo)
    if (obj.id === 'current-balance') {
        if (end < 0) {
            obj.classList.remove('text-gray-100');
            obj.classList.add('text-red-600');
        } else {
            obj.classList.remove('text-red-600');
            obj.classList.add('text-gray-100');
        }
    }
...
/**
 * FIREBASE FUNCTIONS (CRUD & REAL-TIME LISTENER)
 */

// READ: Setup real-time listener for transactions from Firestore
function setupTransactionsListener() {
    try {
        // Query dengan urutan tanggal terbaru dan menggunakan onSnapshot untuk real-time updates
        db.collection("transactions")
          ...
          .onSnapshot(snapshot => {
                ...
                // Perbarui tampilan setiap kali ada perubahan data (otomatis real-time)
                updateDashboard();
                displayTransactions();
                updateReports();
                
                // Opsional: Log untuk konfirmasi update
                console.log("Real-time update received. Total transactions:", transactions.length);

          }, error => {
              // Handler jika terjadi error pada listener
              console.error("Error setting up real-time listener: ", error);
              showNotification('Gagal mendengarkan pembaruan data real-time.', 'error');
          });

    } catch (error) {
        console.error("Error initializing listener: ", error);
        showNotification('Gagal inisialisasi listener Firebase.', 'error');
    }
}
...
async function handleAddTransaction(e) {
    ...
    try {
        // Simpan ke Firestore. Listener (onSnapshot) akan memuat ulang data secara otomatis.
...
async function handleEditTransaction(e) {
    ...
    try {
        // Update di Firestore. Listener (onSnapshot) akan memuat ulang data secara otomatis.
...
async function confirmAndDeleteTransaction(id) {
    ...
    try {
        // Hapus di Firestore. Listener (onSnapshot) akan memuat ulang data secara otomatis.
...
function updateDashboard() {
    ...
    // --- 1. ANIMASI TOTAL SALDO ---
    const balanceElement = document.getElementById('current-balance');
    const currentDisplayedBalance = parseCurrency(balanceElement.textContent);
    
    // Animasikan Saldo (Durasi 1000ms)
    animateValue(balanceElement, currentDisplayedBalance, newBalance, 4000); 

    // --- 2. ANIMASI TOTAL PEMASUKAN ---
    const incomeElement = document.getElementById('total-income');
    const currentDisplayedIncome = parseCurrency(incomeElement.textContent);
    
    // Animasikan Pemasukan (Durasi 800ms)
    animateValue(incomeElement, currentDisplayedIncome, totalIncome, 4000); 

    // --- 3. ANIMASI TOTAL PENGELUARAN ---
    const expenseElement = document.getElementById('total-expense');
    const currentDisplayedExpense = parseCurrency(expenseElement.textContent);
    
    // Animasikan Pengeluaran (Durasi 800ms)
    animateValue(expenseElement, currentDisplayedExpense, totalExpense, 4000); 
